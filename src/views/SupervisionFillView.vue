...
const cropAspect = ref(null)  // null = อิสระ, number = fixed
...
async function loadForm() {
  ...
  const { data, error: rpcErr } = await supabase
    .rpc('get_supervision_form_public', { p_token: token.value })
  ...
  if (form.value.show_responsible && form.value.responsible_ids?.length) {
    const { data: pp } = await supabase.from('profiles')
      ...
      .in('id', form.value.responsible_ids)
...
async function uploadBlob(questionId, blob, ext = 'jpg') {
  ...
  try {
    if (externalUploadEnabled) {
      ...
      evidenceFileUrls.value[questionId] = supabase.storage
        .from('supervision-evidence').getPublicUrl(path).data.publicUrl
    }
  } catch (err) {
...
function onFileChange(questionId, event) {
  ...
  if (file.type === 'application/pdf') {
    uploadBlob(questionId, file, 'pdf')
    return
  }
...
function cancelCrop() {
  cropModal.value = { open: false, questionId: null, imageSrc: '' }
}

function rePickImage(questionId) {
  evidenceFileUrls.value[questionId] = null
  evidencePreviews.value[questionId] = null
  document.getElementById(`file_${questionId}`)?.click()
}
...
// ─── ป้องกัน Teleport ค้างเมื่อ route เปลี่ยน ────────────────────────────────
onUnmounted(() => { cropModal.value.open = false })

// ─── Validation ───────────────────────────────────────────────────────────────
function validate() {
  if (!isIndividual.value && !selectedSchool.value) return 'กรุณาเลือกโรงเรียน'
  ...
  for (const q of questions.value) {
    const idx = questions.value.indexOf(q) + 1
    if (q.required) {
      const a = answers.value[q.id]
    ...
    if (q.evidence_required && q.evidence_type !== 'none') {
      ...
      if (needUpload && !evidenceFileUrls.value[q.id]) return `กรุณาอัปโหลดหลักฐานข้อที่ ${idx}`
      if (needLink   && !evidenceLinks.value[q.id]?.trim()) return `กรุณาใส่ลิงค์หลักฐานข้อที่ ${idx}`
    }
  }
  return null
}
...
async function submit() {
  ...
  const school = isIndividual.value
    ? schools.value.find(s => s.id === respondentSchoolId.value)
    : schools.value.find(s => s.id === selectedSchool.value)
  ...
  const { data, error: rpcErr } = await supabase.rpc('submit_supervision_public', {
    p_token:           token.value,
    p_school_id:       isIndividual.value ? (respondentSchoolId.value || null) : selectedSchool.value,
    p_school_name:     school?.name || null,
  ...
  if (rpcErr) {
    Swal.fire({ icon: 'error', title: 'เกิดข้อผิดพลาด', text: rpcErr.message })
    return
  }
...
<template>
  <div class="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50 font-sarabun">
    <div class="max-w-2xl mx-auto px-4 py-8">
      ...
      <!-- Already submitted -->
      <div v-else-if="alreadyDone" class="text-center py-24">
        ...
        <h2 class="text-lg font-bold text-slate-700 mb-1">ส่งแบบนิเทศไปแล้ว</h2>
        <p class="text-sm text-slate-400">โรงเรียนของคุณได้ส่งแบบนิเทศนี้เรียบร้อยแล้ว</p>
      </div>

      <!-- Submitted success -->
      <div v-else-if="submitted" class="text-center py-24">
      ...
      <template v-else-if="form">
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-slate-100 p-6 mb-6">
          ...
          <h1 class="text-2xl font-extrabold text-slate-800 mt-2">{{ form.title }}</h1>
          <p v-if="form.description" class="text-slate-500 text-sm mt-2">{{ form.description }}</p>
          <div v-if="form.show_responsible && (form.responsible_group || responsiblePeople.length)"
          ...
          <div class="mt-4">
            <div class="flex justify-between text-xs text-slate-400 mb-1">
              <span>ตอบแล้ว {{ answered }} / {{ questions.length }} ข้อ</span>
              <span>{{ progress }}%</span>
            </div>
            ...
            <div v-if="showField('gender')" class="sm:col-span-2">
              ...
              <div class="flex gap-3">
                <label v-for="g in ['ชาย','หญิง','ไม่ระบุ']" :key="g"
                  ...
                  <input type="radio" v-model="respondentGender" :value="g" class="sr-only"/>
                  {{ g }}
                </label>
              </div>
            </div>
        ...
        <div class="space-y-4">
          ...
          <div v-if="q.type === 'section'"
            ...
            <div class="flex items-center gap-2">
              <svg class="w-5 h-5 text-indigo-200 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
          ...
          <div v-else
            ...
            <!-- textarea -->
            <textarea v-else-if="q.type === 'textarea'" v-model="answers[q.id]" rows="3"
              placeholder="พิมพ์คำตอบ..."
              class="w-full px-4 py-2.5 border border-slate-200 rounded-xl text-sm resize-none focus:outline-none focus:border-primary"/>

            <!-- number -->
            <input v-else-if="q.type === 'number'" v-model.number="answers[q.id]" type="number"
              placeholder="0"
              class="w-full px-4 py-2.5 border border-slate-200 rounded-xl text-sm focus:outline-none focus:border-primary"/>
            ...
            <div v-else-if="q.type === 'rating'" class="flex flex-wrap gap-2">
              <button v-for="n in (q.settings?.max || 5)" :key="n" type="button"
            ...
            <!-- ─── หลักฐานอ้างอิง ────────────────────────────────────────── -->
            <div v-if="q.evidence_type && q.evidence_type !== 'none'"
              class="mt-4 pt-3 border-t border-slate-100 space-y-3">
              ...
              <div v-if="['upload','both'].includes(q.evidence_type)">
                ...
                <!-- ยังไม่มีภาพ: ปุ่มเลือกไฟล์ -->
                <label v-if="!evidencePreviews[q.id] && !uploadingEvidence[q.id]"
                  :for="`file_${q.id}`"
                  ...
                  <span>เลือกภาพ / ถ่ายภาพ / ไฟล์ PDF</span>
                </label>

                <!-- กำลังอัปโหลด -->
                <div v-else-if="uploadingEvidence[q.id]"
                  class="flex items-center gap-2 px-4 py-3 bg-blue-50 rounded-xl text-sm text-blue-700">
                  <svg class="w-4 h-4 animate-spin flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"/>
                  </svg>
                  กำลังอัปโหลด...
                </div>

                <!-- มีภาพแล้ว: แสดง preview + ปุ่มเปลี่ยน -->
                <div v-else-if="evidencePreviews[q.id]" class="flex items-start gap-3">
                  <img :src="evidencePreviews[q.id]" alt="หลักฐาน"
                    class="w-24 h-24 object-cover rounded-xl border border-slate-200 shadow-sm flex-shrink-0"/>
                  <div class="flex flex-col gap-1.5 pt-1">
                    ...
                    <p class="text-xs text-slate-400">ปรับขนาดแล้ว ≤ 1200px · JPEG 75%</p>
                    <button type="button" @click="rePickImage(q.id)"
                      class="text-xs text-primary font-bold hover:underline text-left">
                      เปลี่ยนภาพ
                    </button>
                  </div>
                </div>
        ...
        <div class="mt-6 pb-8">
          ...
          <p class="text-center text-xs text-slate-400 mt-3">
            ข้อมูลจะถูกส่งให้กับ สพป. เพื่อประมวลผลต่อไป
          </p>
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-lg flex flex-col overflow-hidden"
          ...
          <div class="px-5 py-3 border-b border-slate-100 flex-shrink-0">
            ...
            <div class="flex flex-wrap gap-1.5">
              <button v-for="p in RATIO_PRESETS" :key="p.label" type="button"
                @click="cropAspect = p.value"
          ...
          <div class="px-5 py-4 border-t border-slate-100 flex items-center justify-between gap-3 flex-shrink-0">
            <p class="text-xs text-slate-400">บีบอัด JPEG 75% · ≤ 1,200 px</p>
            <div class="flex gap-2">
              ...
              <button type="button" @click="confirmCrop" :disabled="cropping"
                class="flex items-center gap-1.5 px-5 py-2 text-sm font-bold text-white bg-primary rounded-xl hover:-translate-y-0.5 disabled:opacity-50 transition-all shadow-md">
