...
const { pendingCount, fetchPendingCount } = useMyTasksBadge()
...
const NAV_ICONS = {
  ...
  // 3 คีย์นี้ ICON_MAP อ้างถึงมาตั้งแต่ระบบผลงานและรางวัลแต่ไม่เคยมีในนี้
  // ผลคือ :d ได้ undefined ไอคอนเลยว่างเปล่า เพิ่มให้ครบทีเดียว
...
const ICON_MAP = {
  ...
  '/dashboard/library':      'documents',
  '/dashboard/document-tasks': 'tasks',
  '/dashboard/contact-messages': 'mail',
  '/dashboard/works':        'works',
  '/dashboard/works-approve':'approve',
  '/dashboard/videos':       'news',
  '/dashboard/videos-approve':'approve',
...
const allGroups = [
  ...
  },
  {
    ...
    items: [
      ...
      { to: '/dashboard/document-tasks', label: 'ระบบธุรการ',    roles: ['super_admin','admin','supervisor','staff'] },
  ...
  },
  {
    ...
    items: [
      ...
      { to: '/dashboard/videos',         label: 'วีดิทัศน์การศึกษา', roles: ['super_admin','admin','supervisor','staff','teacher','school'] },
      { to: '/dashboard/videos-approve', label: 'อนุมัติวีดิทัศน์',  roles: ['super_admin','admin','supervisor'] },
    ...
    <aside :class="[
      ...
      <div class="border-t border-white/10 p-3 flex-shrink-0">
        <div v-if="profile"
          ...
          <div class="relative w-8 h-8 rounded-lg flex items-center justify-center text-xs font-extrabold flex-shrink-0 text-white"
            style="background-color: var(--color-secondary)">
            {{ (profile.full_name || 'U')[0].toUpperCase() }}
            <span v-if="pendingCount > 0" class="absolute -top-1 -right-1 w-2.5 h-2.5 bg-red-500 rounded-full ring-2 ring-[#0c1220]" title="มีงานเอกสารค้าง"></span>
          </div>
      ...
      <header class="sticky top-0 z-20 h-16 glass-nav flex items-center justify-between px-4 gap-4">
        ...
        <div class="flex items-center gap-2">
          ...
          <div v-if="profile"
            ...
            <div class="relative w-6 h-6 rounded-lg flex items-center justify-center text-[10px] font-extrabold text-white flex-shrink-0"
              style="background-color: var(--color-secondary)">
              {{ (profile.full_name || 'U')[0].toUpperCase() }}
              <span v-if="pendingCount > 0" class="absolute -top-1 -right-1 w-2.5 h-2.5 bg-red-500 rounded-full ring-2 ring-slate-50" title="มีงานเอกสารค้าง"></span>
            </div>
