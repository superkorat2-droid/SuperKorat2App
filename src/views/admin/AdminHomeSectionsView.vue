...
const SECTION_ICONS = {
  ...
  dmc_stats:       'M3.75 3v11.25A2.25 2.25 0 006 16.5h2.25M3.75 3h-1.5m1.5 0h16.5m0 0h1.5m-1.5 0v11.25A2.25 2.25 0 0118 16.5h-2.25m-7.5 0h7.5m-7.5 0l-1 3.75m8.5-3.75l1 3.75m-9.5 0h10.5',
  nt_scores:       'M9 17.25v1.007a3 3 0 01-.879 2.122L7.5 21h9l-.621-.621A3 3 0 0115 18.257V17.25m6-12V15a2.25 2.25 0 01-2.25 2.25H5.25A2.25 2.25 0 013 15V5.25m18 0A2.25 2.25 0 0018.75 3H5.25A2.25 2.25 0 003 5.25m18 0V12a2.25 2.25 0 01-2.25 2.25H5.25A2.25 2.25 0 013 12V5.25',
}
...
const SECTION_DESC = {
  ...
  dmc_stats:       'สรุปจำนวนนักเรียนทั้งเขตจากรอบ DMC ล่าสุดที่เผยแพร่ต่อสาธารณะ — ลิงก์ไปหน้า /student-stats',
  nt_scores:       'กราฟแนวโน้มผลคะแนน NT รายปีที่แอดมินเลือกเผยแพร่ (ที่หน้ารายการรอบ NT) — ปุ่ม "ดูข้อมูลทั้งหมด" ต้อง login เป็นบุคลากรเขต',
}
...
function addDriveSection() {
  sections.value.push({
    key: `drive_${Date.now()}`, label: 'โฟลเดอร์ Drive', subtitle: 'Documents', title: 'เอกสารดาวน์โหลด', visible: true,
    bg: '#ffffff', bg2: '#f1f5f9', bg_type: 'none', order: sections.value.length + 1,
    drive: { url: '', folder_id: '', title: '', view: 'grid', cols: 4, rows: 3, show_search: true, allow_subfolders: true },
  })
}
...
function addNewsletterSection() {
  sections.value.push({
    key: `school_newsletters_${Date.now()}`, label: 'จดหมายข่าวโรงเรียน', subtitle: 'School Activities',
    title: 'กิจกรรมโรงเรียนในสังกัด', visible: true,
    bg: '#ffffff', bg2: '#f1f5f9', bg_type: 'none', order: sections.value.length + 1,
...
onMounted(async () => {
  ...
  const merged = [
    ...raw.map(s => {
      ...
      // เช่นเดียวกัน — เซกชันจดหมายข่าวที่บันทึกไว้ก่อนมี field newsletters
...
<template>
  <div class="font-sarabun space-y-5">
    ...
    <div class="flex flex-wrap items-center justify-between gap-3">
      ...
      <button @click="addDriveSection" type="button"
        class="flex items-center gap-1.5 px-4 py-2 rounded-xl text-sm font-bold border-2 border-dashed border-slate-300 text-slate-500 hover:border-primary hover:text-primary transition-all">
      ...
      <button @click="addNewsletterSection" type="button"
        class="px-4 py-2.5 rounded-xl text-sm font-bold border-2 border-dashed border-slate-300 text-slate-500
               hover:border-primary hover:text-primary transition-all">
