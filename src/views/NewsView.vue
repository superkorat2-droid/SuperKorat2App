    ...
    <div class="max-w-7xl mx-auto px-4 py-8">
      ...
      <div v-else>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-5">
          <NewsCard v-for="n in paginated.filter(x => page!==1 || !x.is_pinned)" :key="n.id"
            :news="n"
            :category-label="catMeta[n.category]?.label || n.category"
            :category-class="`${catMeta[n.category]?.bg || 'bg-slate-100'} ${catMeta[n.category]?.text || 'text-slate-600'}`"
