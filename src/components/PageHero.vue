...
<template>
  ...
  <div v-else class="relative overflow-hidden" :class="size === 'lg' ? 'min-h-[220px]' : 'min-h-[140px]'">
    <!-- ไล่กรมท่า→ทอง 3 สเต็ป + แสงตกมุมบนซ้าย ให้หัวเพจมีความลึก ไม่ใช่ไล่สีแบนสองสี -->
    <div class="absolute inset-0" style="background:
      radial-gradient(62% 115% at 14% -5%, rgba(255,255,255,0.18) 0%, transparent 58%),
      linear-gradient(135deg, var(--color-primary-deep) 0%, var(--color-primary) 46%, var(--color-secondary) 100%)"></div>
    ...
    <div :class="[containerMaxW, 'relative mx-auto px-4 flex flex-col',
      ...
      <div v-if="showIcon" :class="[
        'inline-flex items-center justify-center rounded-2xl bg-white/20 backdrop-blur-md ring-1 ring-white/35 shadow-lg shadow-black/10',
