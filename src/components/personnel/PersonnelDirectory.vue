<script setup>
/**
 * PersonnelDirectory — ผังบุคลากร (การ์ด + จัดกลุ่ม + โมดัลรายละเอียด)
 *
 * สกัดจาก PersonnelView เพื่อให้ "โค้ดฝังสำหรับเว็บภายนอก" (/embed/personnel)
 * ใช้โครงสร้างเดียวกันเป๊ะ — ถ้าแยกกันเขียน วันหน้าแก้หน้าบุคลากรแล้วโค้ดฝังจะไม่ตาม
 *
...
const directors = computed(() => props.personnel.filter(p => p.org_role === 'director' && isDeptVisible(p)))
const deputies  = computed(() => props.personnel.filter(p => p.org_role === 'deputy' && isDeptVisible(p)))
const groupDirs = computed(() => props.personnel.filter(p => p.org_role === 'group_director' && isDeptVisible(p)))
const nonExecs  = computed(() => props.personnel.filter(p => !['director','deputy','group_director'].includes(p.org_role)))
